...
The dataset provided included 870 observations and 36 features. Below, you will find the models that I built to predict attrition, as well as salary. We identified key variables that play the biggest roles in predicting both attrition and salary. The top three for attrition, that I found, were NumCompaniesWorked, YearsSinceLastPromotion, DistanceFromHome. The top three for salary were JobLevel, JobRole, TotalWorkingYears.
...
```{r}
# convert to factors
frito$EnvironmentSatisfaction <- factor(frito$EnvironmentSatisfaction)
...
frito$RelationshipSatisfaction <- factor(frito$RelationshipSatisfaction)
...
sample <- sample(c(TRUE, FALSE), nrow(frito), replace = TRUE, prob = c(0.7, 0.3))
train <- frito[sample, ]
test <- frito[!sample, ]
...
fit2 <- lm(attritionDummy ~ NumCompaniesWorked + 
...
```{r}
frito %>% select('DistanceFromHome', 'NumCompaniesWorked', 'YearsSinceLastPromotion', 'Attrition') %>% ggpairs(mapping = ggplot2::aes(colour=Attrition))
...
# convert to factors
noAttrition$EnvironmentSatisfaction <- factor(noAttrition$EnvironmentSatisfaction)
...
noAttrition$PerformanceRating <- factor(noAttrition$PerformanceRating)
noAttrition$RelationshipSatisfaction <- factor(noAttrition$RelationshipSatisfaction)
...
# Write to csv
write.csv(attrition_pred,"C:\\Users\\austi\\Downloads\\Case2PredictionsWebbAttrition.csv", row.names = FALSE)
...
# convert to factors
noSalary$EnvironmentSatisfaction <- factor(noSalary$EnvironmentSatisfaction)
...
noSalary$RelationshipSatisfaction <- factor(noSalary$RelationshipSatisfaction)
...
# Write to csv
write.csv(salary_pred,"C:\\Users\\austi\\Downloads\\Case2PredictionsWebbSalary.csv", row.names = FALSE)
